import pandas as pd
import pytz
import joblib  # Kept for compatibility, though not used here
import os
import pickle
import json
...
MODEL_PATH = "op_model.pkl"
TIMESTAMP_PATH = "last_op_training_timestamp.pkl"
...
def train_op_model():
    #Train a model to analyze outpatient visit patterns.
    ...
    if last_timestamp is None:
        ...
        INNER JOIN master.m_employee_details me ON me.id = tv.doctor_id AND me.active = TRUE
        INNER JOIN master.m_department md ON md.id = tv.department_id
        WHERE tv.updated_date > %s AND tv.visit_type_id IN (1, 5);
        """
    ...
    new_visits = len(df)
    total_op_visits = new_visits
    if previous_result and 'total_op_visits' in previous_result:
        total_op_visits = previous_result['total_op_visits'] + new_visits
    ...
    if df.empty:
        if previous_result:
            ...
            return top_days, peak_hour_ranges, 0.0, top_doctors, top_departments, previous_result.get('total_op_visits', 0)
    ...
    # Identify peak hours using 85th percentile threshold
    ...
    return top_days, peak_hour_ranges, accuracy if 'accuracy' in locals() else 0.0, top_doctors, top_departments, total_op_visits
